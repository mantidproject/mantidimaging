...
def main_job(argv):
    ...
    past = ''
    try:
        opts, args = getopt.getopt(argv, "hu:ls:n:g:p:",
                                   ['help', 'user=', 'long', 'status=', 'name=', 'group=', 'past='])
    except getopt.GetoptError:
        job_usage()
        return
    ...
    if len(args) > 0:
        ...
        if (len(jobId) == 0) | ((p.match(jobId.lower()) == None) and
                                (pl.match(jobId.lower()) == None)) | (len(args) > 1):
            job_usage()
            return
    if (len(jobStatus) > 0 and len(jobName) > 0) | (len(jobStatus) > 0 and len(jobId) > 0) | (
            len(jobId) > 0 and len(jobName) > 0) | (len(jobId) > 0 and len(group) > 0) | (
                len(jobName) > 0 and len(group) > 0) | (len(jobStatus) > 0 and len(group) > 0):
...
def showJobinfo(jobs, long):
    if long == '':
        ...
        for xdoc in jobs:
            ...
            cmd = xdoc.find('cmd')
            print '%-10s%-10s%-23s%-25s%s' % (jobId, checkFieldValidity(status), SubStr(
                checkFieldValidity(extStatus)), SubStr(name), checkFieldValidity(cmd))
    else:
        for xdoc in jobs:
            jobId = xdoc.find('id').text
            name = xdoc.find('name').text
