...
class ShapeSplitter(object):
    ...
    def _determine_dtype_size(self, dtype=None):
        ...
        if dtype in ['int16', 'float16', 'np.int16', 'np.float16', '16'] or isinstance(dtype, np.int16):
            return 16
        elif dtype in [ 'int32', 'float32', 'np.int32', 'np.float32', '32'] or isinstance(dtype, np.float32):
            return 32
        elif dtype in [ 'int64', 'float64', 'np.int64', 'np.float64', '64'] or isinstance(dtype, np.float64):
            return 64
    ...
    def calc(self):
        assert self.axis < len(self.shape), "The required axis is outside the shape of the data!"
        ...
        print("Single size of shape {0} on axis {1}: {2}\nTotal size: {3}\nAllowed memory: {4}"
            .format(self.shape, self.axis, single, self.full_size(), self.max_mem))

        full_size = self.full_size()
        ratio = self.R(full_size)
        ratio = np.ceil(ratio)

        split, step = np.linspace(0, length, ratio, dtype=np.int32, retstep=True)
        new_shape = self.shape[:self.axis]+(int(step),)+self.shape[self.axis+1:]
        while self.R(self.full_size(new_shape)) > self.max_ratio:
            split, step = np.linspace(0, length, ratio, dtype=np.int32, retstep=True)
            new_shape = self.shape[:self.axis]+(int(step),)+self.shape[self.axis+1:]
            ratio += 1
            print("Step per reconstruction", step, "with a ratio to memory", self.R(self.full_size(new_shape)), "indices", split)
...
if __name__=='__main__':
    ...
    s = ShapeSplitter((axis_0, axis_1, axis_2), 0, dtype, max_mem, max_ratio=max_ratio)
    print(s.calc())