...
def execute(config):
    ...
    do_aggregating(angle_image_paths, img_format, agg_method, energies_label, single_folder, h)
...
def do_aggregating(angle_image_paths, img_format, agg_method, energies_label, single_folder, h):
    ...
    for angle, image_paths in angle_image_paths:
        ...
        h.pstart("Loading data for angle {0} from path {1}".format(angle, os.path.dirname(image_paths[0])))
        images = loader.load(
            file_names=image_paths, img_format=img_format)[0]
        # sum or average them
        if 'sum' == agg_method:
