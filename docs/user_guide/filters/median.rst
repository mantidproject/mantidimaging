Median
======

Applies SciPy's median filter (:code:`scipy.ndimage.filters.median_filter`)
(docs_).

There is the option to run the median filter on the GPU if :code:`cupy` is installed and working on your system. For more information see :doc:`GPU Mode <gpu_mode.rst>`.

Python API
----------

.. autoclass:: mantidimaging.core.filters.median_filter.MedianFilter

.. _docs: https://docs.scipy.org/doc/scipy-0.16.1/reference/generated/scipy.ndimage.filters.median_filter.html
