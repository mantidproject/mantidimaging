...
Users can now view convergence and residual plots within the reconstruction view, providing deeper insight into the reconstruction process. Additionally, initial support for geometry handling has been introduced.

**Live Viewer Improvements:**
The live viewer tool now supports opening multiple windows simultaneously, allowing users to monitor several datasets at once. Displaying date and time information for newly acquired images makes it easier to track and correlate data as it arrives. The addition of a new line plot for intensity visualisation provides immediate feedback on the image quality and experiment conditions. In addition to the new feature added to the live viewer, Mantid Imaging 3.0.0 also focuses on improved performance making the live viewer more flexible and informative for users working with real-time data.

**Enhanced Logging:**
In addition to numerous bug fixes in Mantid Imaging 3.0.0, a new logging view is now available from the Settings dialog. This feature enables users to set the desired log level, making it easier to provide detailed error reports to the Mantid Imaging team.
...
- `#2420 <https://github.com/mantidproject/mantidimaging/issues/2420>`_: corrected RITS export error when bin size equals step size
...
- `#2642 <https://github.com/mantidproject/mantidimaging/issues/2642>`_: While using the Santisteban fit, the window will not crash if a bad fit is found or a poor ROI is chosen. The user receives a warning if a poor ROI is used.
...
- `#2268 <https://github.com/mantidproject/mantidimaging/issues/2268>`_: various python packages have been updated including astropy, scipy, nu,py, algotom, cudatoolkit, cupy, and jenkspy
...
- `#2289 <https://github.com/mantidproject/mantidimaging/issues/2289>`_: updated packages to deal with numpy deprecation warnings leading towards 2.0
