...
def getTranslations():
	...
	return ( gettext.translation (APP_NAME,
									mo_location,
									languages = languages,
									fallback = True) )
...
def downloadFile(srcName, dstPath, jobId, cmd):
	...
	headers = {'Content-Type': 'text/plain', 'Cookie': token, 'Accept': ACCEPT_TYPE, 'Accept-Language': getSysLocale().replace("_", "-").lower()}
	try:
...
def uploadLargeFile(jobId, dstPath, dFile):
	...
	while fileSize > 0:
		...
		if (fileSize - chunksize) <= 0:
			chunksize = fileSize
		fileContent = fileobj.read(chunksize)
		fileSize -= chunksize
		try:
...
def logon(url, username, password):
	...
	if ( (x509Flag == False) | (len(username) > 0) ):
		...
		headers = {'Content-Type': 'application/xml', 'Cookie': token, 'Accept': ACCEPT_TYPE, 'Accept-Language': getSysLocale().replace("_", "-").lower()}
		body='<User><name>%s</name> <pass>%s</pass> </User>' % (username, password)
	else:
		headers = {'Content-Type': 'application/xml', 'Accept': ACCEPT_TYPE, 'Accept-Language': getSysLocale().replace("_", "-").lower()}
		body='<User></User>'
...
def encode_body(boundary, appName, params, inputFiles):
	...
	def encode_file(filepath, filename):
		return('--' + boundary,
			...
			'Accept-Language:' + getSysLocale().replace("_", "-").lower(),
			'Content-ID: <%s>' % urllib2.quote(filename),
...
def getHttp(url,x509Flag):
	...
	if ( (len(pemFile) > 0) & (sslHandshakeRequiredFlag == True) ):
		http = httplib2.Http(ca_certs=pemFile)
	else:
		http = httplib2.Http()
	return http
