...
def main_logon(argv):
	...
	if (len(url) == 0) | (p.match(url.lower()) == None):
		print "Specify the URL of the PAC web services in the format: http://host_name:port_number/[context_path] or https://host_name:port_number/[context_path]"
		return
	url = url + context
	url = removeQuote(url)
...
def showJobinfo(jobs,long):
	if long == '':
		...
		for xdoc in jobs:
			...
			print 'Local Job ID:%s' % checkFieldValidity(localJobId)
			print 'Local Cluster:%s' % checkFieldValidity(localClusterName)
			print 'Job Forwarding:%s' % checkFieldValidity(jobForwarding)
			if checkFieldValidity(jobForwarding) != 'None':
				print 'Remote Job ID:%s' % checkFieldValidity(remoteJobId)
				print 'Remote Cluster:%s' % checkFieldValidity(remoteClusterName)		
			if len(jobs)>1:
				print ' '
				print ' '
