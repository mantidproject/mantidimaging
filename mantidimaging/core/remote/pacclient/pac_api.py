...
def downloadFile(srcName, dstName, jobId):
    ...
    headers = {'Content-Type': 'text/plain',
               'Cookie': token, 'Accept': ACCEPT_TYPE}
    try:
        response, content = http.request(
            url_file, 'GET', body=body, headers=headers)
        if len(content) > 0:
            f = open(dstName, 'wb')
            f.write(content)
            f.close()
...
def downloadJobFiles(jobId, dstPath, dFile):
    ...
    if os.access(dstPath, os.W_OK) == 0:
        ...
        return
    url_jobfiles = url + 'platform/webservice/pacclient/jobfiles/' + jobId
    headers = {'Content-Type': 'text/plain',
               'Cookie': token, 'Accept': ACCEPT_TYPE}
    try:
        if len(dFile) > 0:
            ...
            response, content = http.request(
                url_jobfiles, 'GET', headers=headers)
...
def uploadJobFiles(jobId, dstPath, dFile):
    ...
    url_upfile = url + 'platform/webservice/pacclient/upfile/' + jobId

    try:
        print "Start to upload..."
        uploadUtil(dstPath, dFile, jobId)
        response, content = http.request(
            url_upfile, 'POST', body=body, headers=headers)
...
def logon(url, username, password):
    ...
    url_logon = url + 'platform/webservice/pacclient/logon/'
    headers = {}
    body = ''
    ...
    try:
        response, content = http.request(
            url_logon, 'GET', body=body, headers=headers)
        if response['status'] == '200':
...
def logout():
    ...
    url_logout = url + 'platform/webservice/pacclient/logout/'
    headers = {'Content-Type': 'text/plain',
               'Cookie': token, 'Accept': ACCEPT_TYPE}
    try:
...
def getJobListInfo(parameter):
    ...
    url_job = url + 'platform/webservice/pacclient/jobs?' + parameter
    headers = {'Content-Type': 'application/xml',
               'Cookie': token, 'Accept': ACCEPT_TYPE}
    try:
...
def getJobForStatus(jobStatus):
    ...
    url_job = url + 'platform/webservice/pacclient/jobsforstatus/' + jobStatus
    headers = {'Content-Type': 'application/xml',
               'Cookie': token, 'Accept': ACCEPT_TYPE}
    try:
        response, content = http.request(url_job, 'GET', headers=headers)
...
def submitJob(jobDict):
    ...
    headers = {'Content-Type': 'multipart/mixed; boundary=' + boundary,
               ...
               'Content-Length': str(len(body))}
    url_submit = url + 'platform/webservice/pacclient/submitapp'
    print body
    #url_submit = 'http://localhost:8080/platform/webservice/pacclient/submitapp'
    try:
        response, content = http.request(
            url_submit, 'POST', body=body, headers=headers)
        if response['status'] == '200':
            xdoc = minidom.parseString(content)
...
def doJobAction(jobAction, jobId):
    ...
    url_jobaction = url + 'platform/webservice/pacclient/jobOperation/' + \
        jobAction + '/' + jobId
    headers = {'Content-Type': 'text/plain',
               'Cookie': token, 'Accept': ACCEPT_TYPE}
    try:
...
def doUserCmd(userCmd):
    ...
    try:
        response, content = http.request(
            url_usercmd, 'POST', body=body, headers=headers)
        if response['status'] == '200':
            xdoc = minidom.parseString(content)
            msg = ''
            if ERROR_TAG in content:
...
def ping(url):
    ...
    try:
        response, content = http.request(
            url_ping, 'GET', body=body, headers=headers)
        if response['status'] == '200':
            print content
        else:
...
def getAppParameter(appName):
    ...
    try:
        response, content = http.request(
            url_app, 'GET', body=body, headers=headers)
        if response['status'] == '200':
            xdoc = minidom.parseString(content)
            if ERROR_TAG in content:
                err = xdoc.getElementsByTagName(ERROR_STR)
