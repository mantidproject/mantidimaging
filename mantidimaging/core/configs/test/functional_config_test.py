...
class FunctionalConfigTest(unittest.TestCase):
    ...
    def test_config_doctest(self):
        self._compare_dict_to_str(self.func_config.__dict__, str(self.func_config), FunctionalConfig)
    ...
    def _compare_dict_to_str(self, class_dict, class_str, this_config):
        ...
        self.assertEquals(
            dictlen, strlen, "Different size of __dict__({0}) and __str__({1}). "
            "Some of the attributes in the {2} are not documented!".format(dictlen, strlen, this_config))
