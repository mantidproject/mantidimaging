...
def execute(data_length, slice_ids, cors_for_sinograms):
    """
    Interpolates the Centers of Rotation for the sinograms that are not provided. This expects CORs for only a
    few sinograms and will interpolate the rest, so that every single sinogram has an associated COR with it.
    ...
    assert cors_for_sinograms and all(
        isinstance(cor, float) for cor in cors_for_sinograms), "The centers of rotation MUST be floats"

    assert slice_ids and all(isinstance(slice, int) for slice in
                             slice_ids), "The sinograms associated with the centers of rotation MUST be ints"
