...
class Geometry(AcquisitionGeometry):
    ...
    def __init__(self,
        ...
        if type == GeometryType.PARALLEL3D:
            ...
            conebeam_3d = self.create_Cone3D(*args,
                                             source_position=[0, 550, 0],
                                             detector_position=[0, -230, 0],
                                             # source_position=[0, 397.8, 0],
                                             # detector_position=[0, -171.16, 0],
                                             units=units,
                                             **kwargs)
            self.config = conebeam_3d.config

        self.set_panel(num_pixels=num_pixels, pixel_size=pixel_size)
        self.set_angles(angles=range(0, 360), angle_unit=angle_unit)
    ...
    def detector_position(self) -> float:
        return self.config.system.detector.position[1]
