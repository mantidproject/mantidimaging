...
def find_if_latest_version(action: Callable[[str], None]):
    ...
    if "unstable" in local_label:
        return _do_version_check(parsed_local_version, _parse_version(remote_unstable_version), action)
    elif "main" in local_label:
        return _do_version_check(parsed_local_version, _parse_version(remote_main_version), action)
    else:
...
def _do_version_check(local: ParsedVersion, remote: ParsedVersion, action: Callable[[str], None]):
