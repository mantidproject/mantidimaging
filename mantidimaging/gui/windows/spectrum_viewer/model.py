...
class SpectrumViewerWindowModel:
    ...
    @staticmethod
    def _roi_to_tuple(roi: "SensibleROI") -> tuple[int, int, int, int]:
        return (roi.left, roi.top, roi.right, roi.bottom)

    @staticmethod
    def _make_cache_key(
            roi: "SensibleROI | tuple[SensibleROI, SensibleROI]",
            mode: "SpecType",
            normalise_with_shuttercount: bool,
            chunk_start: int,
            chunk_end: "int | None",) -> tuple:

        if isinstance(roi, tuple):
            sample_roi, open_beam_roi = roi
            sample_roi_key = SpectrumViewerWindowModel._roi_to_tuple(sample_roi)
            open_beam_roi_key = SpectrumViewerWindowModel._roi_to_tuple(open_beam_roi)
            roi_key = (sample_roi_key, open_beam_roi_key)
        else:
            roi_key = SpectrumViewerWindowModel._roi_to_tuple(roi)

        return (roi_key, mode, normalise_with_shuttercount, chunk_start, chunk_end)
    ...
    def get_spectrum(self,
        ...
        cache_key = self._make_cache_key(roi, mode, normalise_with_shuttercount, chunk_start, chunk_end)
        ...
        if isinstance(roi, tuple) and len(roi) == 2:
            sample_roi, open_beam_roi = roi
        else:
            sample_roi = open_beam_roi = roi

        if mode == SpecType.SAMPLE:
            sample_spectrum = self.get_stack_spectrum(self._stack, sample_roi, chunk_start, chunk_end)
            self.spectrum_cache[cache_key] = sample_spectrum
        ...
        if mode == SpecType.OPEN:
            ...
            self.spectrum_cache[cache_key] = open_spectrum
            return open_spectrum

        if mode == SpecType.SAMPLE_NORMED:
            ...
            spectrum = np.divide(
                sample_spectrum,
                open_spectrum,
                out=np.zeros_like(sample_spectrum),
                where=open_spectrum != 0, )

            if normalise_with_shuttercount:
                shutter_corr = self.get_shuttercount_normalised_correction_parameter()
                spectrum = spectrum / shutter_corr

            self.spectrum_cache[cache_key] = spectrum
            return spectrum
