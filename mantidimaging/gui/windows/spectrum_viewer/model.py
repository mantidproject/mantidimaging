...
class SpectrumViewerWindowModel:
    ...
    def _make_cache_key(
        ...
        roi_key: tuple[tuple[int, int, int, int], tuple[int, int, int, int] | None]

        if isinstance(roi, tuple):
            sample_roi, open_beam_roi = roi
            roi_key = (
                SpectrumViewerWindowModel._roi_to_tuple(sample_roi),
                SpectrumViewerWindowModel._roi_to_tuple(open_beam_roi),
            )
        else:
            roi_key = (
                SpectrumViewerWindowModel._roi_to_tuple(roi),
                None,
            )
