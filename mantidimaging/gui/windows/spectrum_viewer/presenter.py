...
class SpectrumViewerWindowPresenter(BasePresenter):
    ...
    def on_initial_params_edited(self) -> None:
        ...
        if self.view.fittingDisplayWidget.is_initial_fit_visible():
            ...
            fit_params, rss, rss_per_dof = self.model.fitting_engine.find_best_fit(xvals, spectrum, init_params, params_bounds=fixed_params)
        ...
        for roi_name, roi_widget in self.view.spectrum_widget.roi_dict.items():
            ...
            try:
                result, rss, rss_per_dof = self.fit_single_region(spectrum, fitting_region, self.model.tof_data,
                                                                  init_params, bounds=fixed_params)
                status = "Fitted"
