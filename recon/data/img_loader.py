...
from recon.data.loader import get_file_names, parallel_move_data, do_stack_load_par, do_stack_load_seq, load_stack
import numpy as np
...
def _load_and_avg_data(load_func, file_path, img_shape, img_format, data_dtype, prog_prefix=None, cores=8,
                       chunksize=None,
...
def _do_files_load_par(data, load_func, files, cores, chunksize, name, h):
    ...
    f = ptsm.create_partial(_par_inplace_load_fwd_func,
                            ptsm.inplace_fwd_func, load_func=load_func)
    ptsm.execute(data, files, f, cores, chunksize, name, h)
    return data


def _load_files(load_func, files, img_shape, dtype, name=None, cores=8, chunksize=None, parallel_load=False, h=None):
    """
